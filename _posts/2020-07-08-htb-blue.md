...
![](/images/blue/blue.png "Blue Info Card")
...
![](/images/blue/4. rapid7_.png "rapid7")
...
![](/images/blue/9. msf_4_exploit_fails_2.png "msf_fails_again")
...
Since we know from nmap that this is a Windows 7 machine, we are going to use 42315.py, so make a copy to your local directory. In this case I am calling it “blue.py”. Once you make the copy, open it in your text editor of choice.
...
![](/images/blue/16. blue.py.png "blue.py usage")
...
The shortest description I've seen is in the null-byte walkthrough: “Named pipes are a way for running processes to communicate with each other with very little overhead.”
...
So now our exploit command should look like this:
    ...
    blue.py 10.10.10.40 netlogon

But we aren't ready yet. One thing we didn't check is what this python script is actually doing. If we read the description, you'll see it is a proof of concept script, so all it does is drop a .txt file on the target to prove execution.  
...
Changes: commented out liens 916-920.  Un-commented and changed lines 922 and 923.  Added commented line 924.
...
![](/images/blue/25. blue.py.exploit.png "run the blue.py exploit")

So even though it looks like we received an error...over in our seond terminal window we see the connection:
...
4. Microsoft Windows named pipe documentation: [https://docs.microsoft.com/en-us/windows/win32/ipc/named-pipes](https://docs.microsoft.com/en-us/windows/win32/ipc/named-pipes)
