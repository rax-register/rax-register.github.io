       ...
       # Nim/C++ Process-injection and Bypass of Windows Defender
...
During the guide we will install the following additional software on the Development Machine:
    ...
    Nim version 1.4.8 and our required Nim packages
    MingW compiler from the Nim website
...
C:\Users\register> path

![](/images/nim_proc_inject_pt1/29. cmd_path.png "")

PowerShell:
PS C:\Users\register> $env:Path
...
C:\Users\register> nimble install winim zippy nimcrypto
...
As we walk through the code and basic explanation, I include comments.  I prefer to pre-comment blocks of code that follow the logical sequence of what I will write, so for our av_bypass program I start with a shell that looks like this:
...
If you have your own method, please use it.  I have found the above process works for me.  As I finish a block of code, I remove the begin / end comments.
...
In the below code snippet, I first describe the overall purpose of the program, followed by an outline and some specific compile notes.  There will be more comments throughout the code, but I like to leave this type of general information at the top.
...
In Part Two of this post, I will delve deeper into what each section of this code is doing, but as an overview, this Nim-wrapped C++ code detects the handles of the current process (our av_bypass.exe) and the ntdll.dll file loaded on initialization.  It then loads a new instance of the ntdll.dll file in memory, unlinks the old ntdll and re-maps to the new before cleaning up.
...
You will see these functions appear in our Nim code below.
...
Substitute the number of bytes for <payload size in bytes>, so in my case:  var shellcode: array[201391, byte] = [ byte <shellcode> ]
...
In this Powershell terminal, we will issue our compiler command to build our executable.  As an example, if we have a hello.nim file with the contents  :

# begin hello.nim
echo "Hello, World!"
# end hello.nim

We need to issue a nim command to compile our code.  A barebones nim compiler command looks like this:
...
hello.nim  :  Nim file to compile into an executable.

If we executed this compile command on our hello.nim file, it would work, and our executable would print "Hello, World!" to the screen when run.  However, a quick glance at the above does not seem to match what we need.

For starters, we use C++ code in our Nim file so the C compiler command is not the right one.  Running "nim --help" shows us some options, but offers a "--fullhelp" option which provides much more detail.  
...
Whic results in our new av_bypass.exe program with the expected increase in file size.  Transfer the file over to your Victim Machine and execute it. 
...
https://github.com/byt3bl33d3r/OffensiveNim  :  Used two of the files from this awesome repository within the code for what we do here:  shellcode_bin.nim and clr_host_cpp_embed_bin.nim
https://s3cur3th1ssh1t.github.io/A-tale-of-EDR-bypass-methods/  :  Provided the example code to load a fresh copy of ntdll.dll to avoid Windows Defender hooks on procedure calls.  There are many other techniques described in this post worth exploring.
https://www.ired.team/offensive-security/defense-evasion/how-to-unhook-a-dll-using-c++  :  Link stolen from the above EDR-bypass post. 
https://nim-lang.org/faq.html  :  Assisted with compiler optimization commands and general Nim topics since this was my first use of Nim.
https://github.com/khchen/winim  :  This is a Nim library available on Github.  Though briefly mentioned in the post, this repository opens up the world of Windows API for Nim coders.  The readme.md puts it best:  "Winim contains Windows API, struct, and constant definitions for Nim. The definitions are translated from MinGW's Windows headers and Windows 10 SDK headers."
https://docs.microsoft.com/en-us/windows/win32/api/  :  Reference documentation for Windows API.
https://git-scm.com/download/win  :  Git download page for Windows
https://code.visualstudio.com/download  :  Visual Studio Code download page
https://nim-lang.org/install_windows.html  :  Nim download page for Windows
