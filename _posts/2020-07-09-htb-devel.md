...
-4- msfvenom to build a .aspx file to execute a meterpreter reverse shell (could also do this as a simple web shell)
...
-8- msfvenom to build a .aspx file to execute a reverse shell 
...
-12- powershell to transfer files to the Devel
...
We have confirmed Anonymous FTP access and can see a few files and one directory: aspnet_client. We also see two files that look like basic web files.  Let's see what happens when we browse the IP address with Firefox:

![](/images/devel/4. website.png "website")

We have a web page with no text, and what looks like an img file.  Let's view page source:

![](/images/devel/5. view-source.png "view-source")

And right there we see the "welcome.png" file. So it looks like the webserver is running from the same directory we have Anonymous FTP access to. This means we can use FTP to put files into the webserver's directory and then trigger execution via a web browser.
...
Once Visual Studio loads the files, click on the Watson project on the right side (red box)
...
From our systeminfo command earlier, we see our target is 32-bit in this case so we set it to x86:
...
Normally, I prefer to download source code and compile it myself since that gives me better insight into what the code is doing and how I might need to modify it to my specific situation. In this case though, SecWiki's ms11-046.exe is good enough for us to use:
